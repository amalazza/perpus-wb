...
class Transaksi
{
	...
	public function countPeminjaman(array $aData)
    {
        $from = date('Y-m-d', strtotime($aData['from']));
		$end = date('Y-m-d', strtotime($aData['end']));
		$oStmt = $this->oDb->prepare('SELECT COUNT(*) FROM peminjaman WHERE tanggal_pinjam BETWEEN :from AND :end');
        $oStmt->bindValue(':from', $from);
        $oStmt->bindValue(':end', $end);
        $oStmt->execute();
        return $oStmt->fetch();
	...
	public function countDenda(array $aData)
    {
        $from = date('Y-m-d', strtotime($aData['from']));
		$end = date('Y-m-d', strtotime($aData['end']));
		$oStmt = $this->oDb->prepare('SELECT SUM(denda) FROM peminjaman WHERE tanggal_pinjam BETWEEN :from AND :end');
        $oStmt->bindValue(':from', $from);
        $oStmt->bindValue(':end', $end);
        $oStmt->execute();
        return $oStmt->fetch();
